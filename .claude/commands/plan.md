---
description: Создать детальный план реализации фичи или изменения
---

# Планирование реализации

Создай план реализации для: $ARGUMENTS

## Задача 0: Определить вход

Если в `$ARGUMENTS` указан путь к артефакту скаута (`_bmad-output/planning-artifacts/[DD_MM_YYYY]_[feature]-scout.md`) — прочитай его как контекст.
Если указано текстовое описание — используй его напрямую.
Если аргументов нет — используй контекст текущего разговора.

## Задача 1: Понять требования

Разбери запрос и определи:
- Основные требования (must-have)
- Желательные фичи (nice-to-have)
- Вне скоупа
- Критерии успеха

## Задача 2: Анализ текущего состояния

1. Прочитай стандарты проекта (`docs/standards/`) — найди релевантные для затронутой области
2. Прочитай релевантный существующий код
3. Определи паттерны, которым нужно следовать
4. Отметь техдолг, который стоит закрыть
5. Проверь существующие тесты

## Задача 3: Создать план реализации

Запиши план в `_bmad-output/planning-artifacts/[DD_MM_YYYY]_[feature]-plan.md` (где `[DD_MM_YYYY]` — сегодняшняя дата в формате `день_месяц_год` через подчёркивания, получи её через Bash `date +%d_%m_%Y`; `[feature]` — то же название, что в scout-артефакте) И выведи его содержимое в терминал.

Префикс даты обязателен — он даёт хронологию артефактов в IDE при сортировке по имени. Дата plan'а может отличаться от даты scout'а — каждый артефакт несёт дату своего создания.

```markdown
# План реализации: [Название фичи]

## Обзор
[1-2 предложения]

## Требования
- REQ-1: [требование]
- REQ-2: [требование]

## Файлы для изменения
| Файл | Изменения |
|------|-----------|
| path/to/file.ts | [что менять] |

## Файлы для создания
| Файл | Назначение |
|------|------------|
| path/to/new-file.ts | [назначение] |

## Шаги реализации
1. [ ] Шаг 1: [описание]
2. [ ] Шаг 2: [описание]
3. [ ] Шаг 3: [описание]

## Стратегия тестирования
- Unit-тесты: [что тестировать]
- E2E-тесты: [что тестировать]

## Оценка рисков
| Риск | Вероятность | Митигация |
|------|-------------|-----------|
| [риск] | Низкая/Средняя/Высокая | [как снизить] |

## План отката
[Как откатить, если что-то пойдёт не так]
```

## Задача 4: Чекпоинт

Представь план и спроси:
"План выглядит нормально? Нужны правки перед тем, как начать реализацию?"
