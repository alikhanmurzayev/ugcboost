...
Запиши отчёт разведки в файл `_bmad-output/planning-artifacts/[DD_MM_YYYY]_[feature]-scout.md` (где `[DD_MM_YYYY]` — сегодняшняя дата в формате `день_месяц_год` через подчёркивания, получи её через Bash `date +%d_%m_%Y`; `[feature]` — краткое название фичи на английском, kebab-case) И выведи его содержимое в терминал.

Префикс даты обязателен — он даёт хронологию артефактов в IDE при сортировке по имени.
