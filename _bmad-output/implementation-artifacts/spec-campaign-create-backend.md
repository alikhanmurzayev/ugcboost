...
- 2026-05-06 (review-loop patches): прогон step-04 (3 ревьюера: blind-hunter / edge-case-hunter / acceptance-auditor) выявил 9 patch-уровневых правок, применены в текущем PR'е. (1) `ValidateCampaignName/TmaURL` сменили сигнатуру с `(string) error` на `(string, error)` — возвращают trimmed, чтобы handler не дублировал нормализацию (single source of truth); handler-`strings.TrimSpace` убран. (2) E2E race-test: добавлен `chan struct{}`-барьер для синхронного старта горутин (без него запросы шли sequential — partial-unique 23505 фактически не покрывался); `winnerLock` под `defer Unlock`; `require.Zero(otherStatus)` поднят в первый ассерт чтобы 5xx сразу всплывал в failure-message. (3) E2E success-кейс расширен `FindAuditEntry` + `json.Marshal/Unmarshal` round-trip для проверки конкретных полей `id/name/tma_url/is_deleted/created_at/updated_at` в `audit_logs.new_value` — закрывает AC #4 на e2e уровне. (4) Удалён мёртвый `SetupCampaign` хелпер из `e2e/testutil/campaign.go` (нигде не вызывался; вернётся в чанке #4). (5) `ValidateCampaignTmaURL` обзавёлся security-комментарием про embedding в outbound Telegram-message (escape — обязанность downstream'а). (6) `service/campaign.go` инлайн-комментарий про «log AFTER WithTx never claims success on rolled-back tx». (7) Defensive `uuid.Parse(campaign.ID)` в handler фиксируется этой записью как намеренный (мертв в проде с `gen_random_uuid()`, нужен только под тест corrupted-row → 500). Findings, не относящиеся к этому story (catch-all `*BusinessError`→409, unmapped 23505/23503, Unicode normalisation, partial-unique COLLATE, `updated_at` trigger, audit-allow-list, FK-aware `DeleteForTests`, rate-limit, structured-log при rejection, audit-row test без `contextWithActor`), вынесены в `deferred-work.md`.
