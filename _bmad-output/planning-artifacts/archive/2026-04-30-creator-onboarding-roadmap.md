...
- [x] **18. Бэк-фундамент сущности креатора + retry в Telegram-нотификаторе (18a).** 3 новые таблицы (`creators` / `creator_socials` / `creator_categories`) + 3 repo + расширение `RepoFactory`. Sentinel-ошибки в `domain/creator.go` без user-facing кодов (привяжем в 18.5). Рефактор `Notifier.fire()` через `cenkalti/backoff/v5` (двойной context: outer maxElapsed + per-attempt sub-ctx) — все три существующие нотификации получают retry. Никаких изменений в OpenAPI / service / handler / authz / e2e. После merge в проде — три пустые таблицы и более надёжная отправка существующих сообщений. PR #65. Спека архивирована — `_bmad-output/implementation-artifacts/archive/2026-05-05-spec-creator-foundation.md`
- [x] **18.5. Бэк: approve заявки админом (18b).** Admin-endpoint, переход `moderation → approved`. В одной `WithTx` — UPDATE статуса + INSERT в `creators` (с зеркалом IIN/Telegram/категорий/соцсетей из заявки) + audit + state-history. `approved` — окончательный, обратного перехода нет. Поднимает user-facing коды для трёх sentinel'ов из 18. PR #66 смержен (chunk 20 поехал внутри). Зависит от 18
- [x] **18.6. Бэк: GET /creators/{id} — aggregate read (18c).** Read-only endpoint для админки и будущих экранов креатора. Возвращает `Creator` + `[]Social` + `[]Category` (с резолвом codes против активных словарей в handler). PR #67 смержен. Зависит от 18.5
- [x] **19. Фронт-админка: action approve в drawer на moderation-экране.** Кнопка «Одобрить заявку» с confirm-modal (зеркало reject, без `hasTelegram`-conditional, emerald). 4 422-кода + 403/404/5xx — единый паттерн error handling через `apiError`-banner; double-submit guard. Спека — `_bmad-output/implementation-artifacts/spec-creator-approve-action-frontend.md`. Юнит-тесты + manual smoke (3 сценария) + e2e spec `admin-creator-applications-moderation-approve-action.spec.ts` (Happy with TG / Cancel / Race 422). Зависит от 16 и 18.5
