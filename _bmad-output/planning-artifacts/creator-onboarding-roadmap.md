...
- [~] **6.5. Browser e2e на admin verification flow.** Playwright spec в `frontend/e2e/web/admin-creator-applications-verification.spec.ts`. Critical user flow: login admin → видит список → применяет фильтр → открывает drawer по клику на строку → prev/next → close. Плюс RoleGuard-блок (brand_manager → redirect на `/`). Бэкенд e2e не нужны — list/counts/get уже покрыты в chunks 4–5. Hard-зависимость: `data-testid` уже расставлены в chunk 6 по стандарту `frontend-components.md`. Спека — `_bmad-output/implementation-artifacts/spec-admin-creator-applications-verification-e2e.md`
