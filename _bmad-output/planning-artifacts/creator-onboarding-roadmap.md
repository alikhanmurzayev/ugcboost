...
- [ ] **10. Бэк: manual verify соцсети админом.** Admin-endpoint, действие из карточки заявки. Помечает конкретную соцсеть как `manual`-верифицированную → авто-переход `verification → moderation`, если это первая верификация. Audit + state-history. Зависит от 7
