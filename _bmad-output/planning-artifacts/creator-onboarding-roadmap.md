...
- [ ] **11. TMA: creator-detail ручка + status mapping + чтение verification-данных.** Создание `GET /creators/applications/me` (или эквивалентной creator-detail ручки с initData-auth) — отдаёт код и verified-поля соцсетей для креатора. На стороне TMA — расширение пустого shell'а из chunk 1: visible-mapping 7→6 по state-machine, базовый экран статуса, чтение detail. Зависит от 7
