...
- [~] **1. Привязка Telegram-аккаунта к заявке.** Кнопка на лендинге → автозапуск бота → автовыполнение `/start` со скрытым ID заявки в payload → бот ловит, привязывает Telegram-пользователя к заявке → открытие TMA с экраном статуса. Артефакты: scout `_bmad-output/implementation-artifacts/29_04_2026_creator-telegram-link-scout.md`, plan `_bmad-output/implementation-artifacts/29_04_2026_creator-telegram-link-plan.md`
