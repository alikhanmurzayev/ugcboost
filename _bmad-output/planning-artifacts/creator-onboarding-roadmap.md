...
- [~] **18. Бэк-фундамент сущности креатора + retry в Telegram-нотификаторе (18a).** 3 новые таблицы (`creators` / `creator_socials` / `creator_categories`) + 3 repo + расширение `RepoFactory`. Sentinel-ошибки в `domain/creator.go` без user-facing кодов (привяжем в 18.5). Рефактор `Notifier.fire()` через `cenkalti/backoff/v5` (двойной context: outer maxElapsed + per-attempt sub-ctx) — все три существующие нотификации получают retry. Никаких изменений в OpenAPI / service / handler / authz / e2e. После merge в проде — три пустые таблицы и более надёжная отправка существующих сообщений. Зависит от 17. Спека — `_bmad-output/implementation-artifacts/spec-creator-foundation.md`
