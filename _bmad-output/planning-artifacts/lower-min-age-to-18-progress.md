...
- [x] Шаг 5: `frontend/landing/src/content.ts:94,97` — обе строки `"Тебе 21+"` → `"Тебе 18+"`.
- [x] Шаг 6: `frontend/landing/src/api/client.ts:25` — пример в комментарии «Возраст менее 21 лет» → «Возраст менее 18 лет».
- [x] Шаг 7: `_bmad-output/implementation-artifacts/spec-creator-application-submit.md:204,228` — добавлена пометка `Update 2026-04-28: откатили обратно к 18`; значение константы в § «Сопутствующие изменения на бэке» обновлено.
- [x] Шаг 8: Локальные гейты — все зелёные, см. ниже.
...
- [ ] Шаг 9: Передать на ревью Alikhan. По memory `feedback_no_commits` / `feedback_no_merge` — Claude не коммитит и не пушит сам. Изменения в working tree, ждут ручного review/commit/push.
...
- Спека `spec-creator-application-submit.md` — обновлена в стиле living document: исторический пункт «MinCreatorAge поднят с 18 до 21» сохранён, ниже добавлен `Update 2026-04-28` с откатом.
