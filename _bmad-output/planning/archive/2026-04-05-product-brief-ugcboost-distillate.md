...
- Креаторы — как правило, физические лица, у которых нет ИП, ТОО или статуса самозанятого, что создаёт юридические проблемы для бухгалтерии брендов

## Флоу онбординга креаторов (детально)

### Шаг 1: Подача заявки (лендинг — веб)
- Креатор заполняет форму на лендинге ugcboost.kz
- Поля: имя, фамилия, **ИИН** (индивидуальный идентификационный номер — обязателен для подписания договора через TrustMe), номер телефона, выбор соцсети (Instagram, TikTok, Threads), ссылки на аккаунты, категория/направление контента, город проживания, физический адрес (улица, дом — для доставки продукции). Адрес можно обновить в личном кабинете (креатор может переехать)
...
- Для MVP: аналитика собирается автоматически через LiveDune API, но дашборд может быть реализован в течение недели после мероприятия

## Личный кабинет бренда (детально)

- Создание и редактирование рекламных кампаний, отправка на модерацию
- Просмотр заявок от креаторов на каждую кампанию, одобрение/отклонение
- Отслеживание сдачи работ, просмотр опубликованного контента
- Для MVP (EFW): команда UGCBoost регистрируется как бренд «Eurasian Fashion Week» и работает с креаторами через полноценный интерфейс бренда — это обеспечивает тестирование всего флоу с точки зрения бренда
...
- Управление платформой, настройка параметров
...
- Для MVP (EFW): команда UGCBoost через админку модерирует собственные кампании EFW — полный цикл: бренд-интерфейс (создание кампаний, работа с креаторами) + админ-интерфейс (модерация, контроль)
...
- Сайт: https://trustcontract.trustme.kz/, документация/тарифы: https://trustme.kz/contract/
- Соответствует **ст. 151-152 ГК РК** и Закону «Об электронном документе и ЭЦП»; SMS-подпись = ручная подпись по закону; принимается судами и налоговыми органами РК
- Blockchain-защита: уникальный хэш подписанного документа записывается в блокчейн, исключая последующие изменения
- Audit trail: IP, устройство, время, факт отправки SMS — доказательная база для суда
- Хранение подписанных документов на серверах в РК **бесплатно 3 года**; на документе 3 QR-кода (подпись отправителя, подпись клиента, ссылка на оригинал)

**Флоу подписания с физлицами (креаторы):**
- Необходимые данные: **ФИО, ИИН, номер телефона**
- Система проверяет номер через БМГ (База Мобильных Граждан) и подтягивает данные из ГБД ФЛ (Госбаза данных физлиц) по ИИН
- UGCBoost загружает договор (PDF/Word или шаблон) → указывает данные креатора → креатор получает SMS со ссылкой → открывает в браузере → читает договор → вводит 4-значный код из SMS → подписано
- Креатору **не нужно**: регистрироваться, скачивать приложение, иметь ЭЦП. Ничего не платит

**Флоу подписания с юрлицами (бренды):**
- Необходимые данные: **БИН** (бизнес-идентификационный номер), контактные данные
- Данные автоматически подтягиваются из ГБД ЮЛ (Госбаза данных юрлиц) по БИН
- Способы подписания: SMS, ЭЦП, eGov Mobile, FaceID — на выбор

**Тарифы:**
- Базовый (50 подписаний/мес): 23 300 ₸/мес. Слайдер: 50, 100, 200, 500, 1000 подписаний (чем больше — тем дешевле за подпись)
- 50 подписей = 25 двусторонних договоров (считаются подписи, не документы); если подписывает одна сторона — 50 договоров
- TrustContract Go (упрощённый для малого бизнеса): 19 990 ₸ за 3 мес / 25 договоров
- 6 бесплатных подписаний после регистрации
- **Опции «Базовые»** (все тарифы): SMS-подписание, ЭЦП/eGov Mobile, FaceID, проверка через БМГ
- **Опции «Стандарт»**: массовое подписание/рассылка/расторжение, интеграция с ГБД ЮЛ и ГБД ФЛ, кастомные SMS, проверка контрагента на благонадёжность
- **Опции «Профи»**: интеграция по **API** (открытый API для любой системы)
- Готовые модули для 1С, Bitrix24, amoCRM (установка 30 мин)

**Рекомендация для UGCBoost MVP (200 креаторов):**
- Тариф «Стандарт» или «Профи» — нужны массовое подписание + интеграция с ГБД ФЛ
- Для автоматической отправки договоров из платформы — тариф **«Профи» с API**
- ~200 одноразовых подписей при онбординге + подписи от брендов = ~200-250 подписаний
- Конкретные цены за «Стандарт» и «Профи» нужно запросить у TrustMe (на сайте только базовый)
- Контакт отдела продаж: +7 (707) 870-19-01 (WhatsApp)
...
- **Бренды:** личный кабинет, только веб (сотрудники работают за компьютерами, мобильное не нужно). В MVP входит — для EFW команда UGCBoost использует полноценный интерфейс бренда
- **Админ:** веб-панель
