...
func TestServer_Login(t *testing.T) {
	...
	t.Run("invalid JSON", func(t *testing.T) {
		...
		router := newTestRouter(t, NewServer(auth, nil, nil, nil, nil, ServerConfig{Version: "test-version"},logmocks.NewMockLogger(t)))
...
func TestServer_RequestPasswordReset(t *testing.T) {
	...
	t.Run("always returns 200 even when service logs error", func(t *testing.T) {
		...
		router := newTestRouter(t, NewServer(auth, nil, nil, nil, nil, ServerConfig{Version: "test-version"},log))
