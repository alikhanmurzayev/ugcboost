...
var (
	...
	campaignRemindSigningPath = "/campaigns/" + campaignUUID + "/remind-signing"
)
...
func TestServer_RemindCampaignCreatorsSigning(t *testing.T) {
	t.Parallel()
	...
	t.Run("forbidden for manager", func(t *testing.T) {
		...
		require.Equal(t, http.StatusForbidden, w.Code)
		require.Equal(t, domain.CodeForbidden, resp.Error.Code)
		ccSvc.AssertNotCalled(t, "RemindSigning", mock.Anything, mock.Anything, mock.Anything)
	})
	...
	t.Run("happy path with empty undelivered → 200, captured-input matches request", func(t *testing.T) {
		t.Parallel()
		authz := mocks.NewMockAuthzService(t)
		authz.EXPECT().CanRemindCampaignCreatorsSigning(mock.Anything).Return(nil)
		ccSvc := mocks.NewMockCampaignCreatorService(t)

		// Captured-input pins campaignID and creatorIDs reaching the service.
		// Catches a future regression where the handler swaps the path arg
		// with the body or mutates the slice before delegation.
		var capturedCampaignID string
		var capturedCreatorIDs []string
		ccSvc.EXPECT().RemindSigning(mock.Anything, campaignUUID, []string{creatorAUUID, creatorBUUID}).
			Run(func(_ context.Context, campaignID string, creatorIDs []string) {
				capturedCampaignID = campaignID
				capturedCreatorIDs = creatorIDs
			}).
			Return(nil, nil)
		...
		require.Equal(t, []string{creatorAUUID, creatorBUUID}, capturedCreatorIDs)
	})
