...
func serverWithCreator(t *testing.T, creator CreatorApplicationService, log *logmocks.MockLogger) *Server {
	t.Helper()
	return NewServer(nil, nil, nil, nil, creator, nil, nil, nil, nil, nil, nil, ServerConfig{
...
func TestServer_SubmitCreatorApplication(t *testing.T) {
	...
	t.Run("utm oversize rejected with 422 VALIDATION_ERROR", func(t *testing.T) {
		t.Parallel()
		// 257 characters — strict-server does not enforce maxLength so the
		// handler must guard the column-cap explicitly.
		creator := mocks.NewMockCreatorApplicationService(t)
		// Service must NOT be called; if the validator slips, the missing
		// EXPECT'ed call surfaces here as a panic instead of a silent pass.

		req := validRequest()
		req.UtmSource = pointer.ToString(strings.Repeat("a", 257))
		...
		require.Contains(t, resp.Error.Message, "utmSource")
	})

	t.Run("utm with control chars rejected with 422", func(t *testing.T) {
		t.Parallel()
		// NUL is the typical Postgres-killer; \x1F covers the rest of the
		// ASCII control range. The validator rejects anything below 0x20 or
		// 0x7F up front so the request never reaches the DB.
		creator := mocks.NewMockCreatorApplicationService(t)

		req := validRequest()
		req.UtmCampaign = pointer.ToString("spring\x00campaign")
		...
		require.Equal(t, http.StatusUnprocessableEntity, w.Code)
		require.Equal(t, domain.CodeValidation, resp.Error.Code)
		require.Contains(t, resp.Error.Message, "utmCampaign")
	})
	...
	t.Run("bot username with url-unsafe characters is escaped in deep link", func(t *testing.T) {
		...
		server := NewServer(nil, nil, nil, nil, creator, nil, nil, nil, nil, nil, nil, ServerConfig{
			Version:               "test-version",
...
func serverWithAuthzAndCreatorAndDict(t *testing.T, authz AuthzService, creator CreatorApplicationService, dict DictionaryService, log *logmocks.MockLogger) *Server {
	t.Helper()
	return NewServer(nil, nil, authz, nil, creator, nil, nil, nil, nil, dict, nil, ServerConfig{
		Version:             "test-version",
		TelegramBotUsername: "ugcboost_test_bot",
	}, log)
}
...
func serverWithApproveDeps(t *testing.T, authz AuthzService, creator CreatorApplicationService, campaignSvc CampaignService, log *logmocks.MockLogger) *Server {
	t.Helper()
	return NewServer(nil, nil, authz, nil, creator, nil, campaignSvc, nil, nil, nil, nil, ServerConfig{
		Version:             "test-version",
		TelegramBotUsername: "ugcboost_test_bot",
	}, log)
}
