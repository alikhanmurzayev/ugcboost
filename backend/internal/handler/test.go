...
// TestAuthService is the subset of auth operations the test handler needs.
...
// TestBrandService is the subset of prod BrandService methods the test
...
// uses.
type TestBrandService interface {
...
// TestHandler provides test-only endpoints that back openapi-test.yaml.
...
type TestHandler struct {
	...
	brands     TestBrandService
	tokenStore TokenStore
	adminID    string
}

var _ testapi.ServerInterface = (*TestHandler)(nil)

// NewTestHandler creates a new TestHandler. adminID must be the seed admin's
// user ID (resolved at startup from cfg.AdminEmail); it is written into the
// request context for brand-seed operations so audit rows have a valid actor.
func NewTestHandler(auth TestAuthService, brands TestBrandService, tokenStore TokenStore, adminID string) *TestHandler {
	return &TestHandler{auth: auth, brands: brands, tokenStore: tokenStore, adminID: adminID}
}
...
func (h *TestHandler) GetResetToken(w http.ResponseWriter, r *http.Request, params testapi.GetResetTokenParams) {
