...
- По умолчанию — без комментария. Комментировать только когда WHY реально неочевиден: скрытое ограничение, нелокальный инвариант, workaround под конкретный баг. Объяснять ЧТО при понятном коде — запрещено
- Godoc для экспортированных идентификаторов — однострочник, описывающий контракт. Многострочный godoc — только при неочевидных предусловиях, побочных эффектах или edge-cases
