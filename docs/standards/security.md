...
Каждое поле БД, которое содержит PII или legal-данные (ИИН, ФИО, телефон, адрес, handle, IP, UserAgent, AgreementVersion, document_version), фиксируется в документе "PII inventory" — что хранится, зачем, на каком legal-основании (consent, legitimate interest, legal obligation), какой retention period. Без inventory legal не сможет ответить на DSR (data subject request) — "что вы хранили обо мне".

Inventory живёт в `legal-documents/` (точное имя — `PII_INVENTORY.md` или эквивалент). Новая колонка с PII / legal-данными — обязательно строка в inventory тем же PR'ом.
...
- [major] Новая колонка с PII / legal-данными без записи в `legal-documents/` PII inventory.
