...
function ApplicationBody({ application }: { application: Application }) {
  const { t } = useTranslation(["creatorApplications", "creators"]);
  ...
  return (
    <>
      ...
      <dl className="mt-5 grid grid-cols-2 gap-x-6 gap-y-4">
        <Field
          label={t("creatorApplications:drawer.birthDate")}
          value={`${formatDate(application.birthDate)} · ${age} ${pluralYears(age)}`}
        />
        <Field label={t("creatorApplications:drawer.iin")} value={application.iin} />
        <Field
          label={t("creatorApplications:drawer.phone")}
          value={
            <a
        ...
        />
        <Field label={t("creatorApplications:drawer.city")} value={application.city.name} />
        ...
        {application.stage === "creators" && (
          <div className="col-span-2 grid grid-cols-3 gap-x-6 gap-y-4 border-t border-surface-200 pt-4">
            <Field
              label={t("creators:drawer.rating")}
              value={<StarRating value={application.rating} />}
            />
            <Field
              label={t("creators:drawer.completedOrders")}
              value={
            ...
            />
            <Field
              label={t("creators:drawer.activeOrders")}
              value={
